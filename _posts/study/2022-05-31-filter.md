...
single-channel EEG 데이터는 4초간 측정되었으며, 250 Hz의 sampling rate가 사용되었습니다.   
...
from scipy import fft
  def fft_scipy(x, sfreq): # sfreq = sampling rate
    # FFT
    X = fft.fft(x) / x.shape[-1]
    # FFT magnitude
    fft_magnitude = abs(X)
    # FFT shift
    freqs = fft.fftfreq(x.shape[-1], d=1/sfreq)
...
따라서, tensorflow로 fft 하기 전에 데이터 타입을 **복소수(complex number)로 변경**해야 합니다.
