...
1. FFT
tensorflow의 fft는 float일 경우 에러가 납니다.
...
`scipy.fft.fft` 및 `tf.signal.fft`를 사용하여 고속 푸리에 변환(fft)을 계산할 수 있으며, 하단의 그림과 같은 결과가 나옵니다.

![Appling fast fourier transform with normalization](https://github.com/HayoonSong/Images-for-Github-Pages/blob/main/study/eeg/2022-05-31-filter/fft_normal.png?raw=true){:.aligncenter}   
<center><span style="color:gray; font-size:70%">Double Sided FFT - with normalization</span></center>

또한, fft 결과값을 신호의 길이로 나눠줘야 y 값이 커지는 것을 방지할 수 있다고 합니다.
나누지 않을 경우 즉 normalization을 하지 않을 경우 다음과 y값이 아주 높은 것을 확인하실 수 있습니다.

![Appling fast fourier transform with normalization](https://github.com/HayoonSong/Images-for-Github-Pages/blob/main/study/eeg/2022-05-31-filter/fft.png?raw=true){:.aligncenter}   
<center><span style="color:gray; font-size:70%">Double Sided FFT - without normalization</span></center>

2. FFT magnitude
푸리에 변환을 통해 두 종류의 그래프를 얻을 수 있습니다.
(1) 푸리에 스펙트럼 그래프(Fourier spectrum graph): 주파수에 따른 magnitude 그래프
(2) 위상 그래프(Phase graph): 주파수에 따른 angle 그래프
...
<center><span style="color:gray; font-size:70%">Double Sided FFT - without FFTShift</span></center>
...
<center><span style="color:gray; font-size:70%">Double Sided FFT - with FFTShift</span></center> 
...
<center><span style="color:gray; font-size:70%">One Sided FFT - with FFTShift</span></center> 
