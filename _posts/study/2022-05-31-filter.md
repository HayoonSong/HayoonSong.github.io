...
**3. FFT shift**   
`scipy.fft.fftfreq` 함수는 input의 주파수 구간을 반환해 줍니다.
...
~~~
FFT의 성질 중 하나로, Nyquist Theorem에 의해서 주파수 축은 $−fs/2$에서 $+fs/2$까지의 구간을 갖습니다.(fs는 sampling frequency로써, 시간 축에서 몇 초에 한번 data를 획득했냐를 의미합니다. 예제 데이터의 경우 0.004초에 한번 샘플링 했으므로 fs=250이며, 최대 주파수는 125Hz가 됩니다.)
